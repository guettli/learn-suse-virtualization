...
A: Both are fields on the **`Prometheus` CR** the monitoring operator manages (surfaced through the `rancher-monitoring` addon values). In Harvester it already ships a **`storageSpec`/volumeClaimTemplate** by default — a **~50Gi PVC on `harvester-longhorn`** — so the TSDB **survives pod restarts out of the box**; retention is governed mainly by **`retentionSize`** (default around **50GiB**), with **`retention`** setting an optional time window. Raise the PVC size and `retentionSize`/`retention` to keep more; only if you strip the PVC would data fall back to **emptyDir** and be lost when the pod moves.
