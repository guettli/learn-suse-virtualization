...
**SUSE Virtualization:** There are no per-VM files on a datastore — a VM is a **VirtualMachine CRD** whose spec lives in **etcd**, with its disks as **Longhorn volumes**.
...
**SUSE Virtualization:** The VM's hardware definition lives in the **VirtualMachine resource's `spec`** (stored in etcd), not in a `.vmx` text file.
...
**SUSE Virtualization:** A virtual disk is a single **Longhorn volume** (a PVC), backed by a **VirtualMachineImage**, rather than a descriptor-plus-flat file pair.
...
**SUSE Virtualization:** No per-VM `.vswp` file — memory pressure is handled by the node's **overcommit configuration** together with normal Linux/Kubernetes memory management.
...
**SUSE Virtualization:** Snapshots are **VirtualMachineSnapshot** objects backed by **Longhorn volume snapshots**, not `.vmsd`/`.vmsn` files.
...
**SUSE Virtualization:** No vSphere-style HW compatibility level — a VM uses a KubeVirt **`machineType` (e.g. `q35`)** plus firmware settings instead.
...
**SUSE Virtualization:** The **qemu-guest-agent** in the guest (enabled by default in Harvester) provides the equivalent integration, while **virtio** drivers replace the VMXNET3/PVSCSI paravirtual devices.
...
**SUSE Virtualization:** Same concept — the **guest OS** runs inside the KubeVirt VM on the node's KVM, and choosing it (via a VM template/preference) sets sensible device defaults.
...
**SUSE Virtualization:** Longhorn volumes are **thin-provisioned**; redundancy is set by the **replica count** rather than by choosing a thick vs thin disk format.
...
**SUSE Virtualization:** **virtio** paravirtual devices (**virtio-net** and **virtio-blk/scsi**) are the equivalent host-aware, high-performance adapters.
...
**SUSE Virtualization:** No vNVMe controller — low-overhead disk I/O is delivered instead through **virtio-blk/virtio-scsi** devices.
...
**SUSE Virtualization:** KubeVirt supports **CPU and memory hot-plug** for running VMs where the guest OS allows it, the equivalent of hot-add.
...
**SUSE Virtualization:** CPU layout is set via KubeVirt's **`spec.domain.cpu`** (sockets/cores/threads); there is no per-socket hypervisor licensing model.
...
**SUSE Virtualization:** KubeVirt provides an **emulated vTPM 2.0** device (non-persistent in Harvester), enabling guest features like BitLocker or Secure Boot measurements.
...
**SUSE Virtualization:** No direct RDM equivalent — the closest options are attaching **Longhorn volumes** or using **PCI/device passthrough** for direct hardware access.
...
**SUSE Virtualization:** KubeVirt VMs boot in **BIOS or UEFI** firmware mode (UEFI enabling **Secure Boot**), the same firmware choice made in the VM's spec.
