...
**SUSE Virtualization:** **No full equivalent** — Harvester has no SDN platform; **Multus** provides multiple VM networks and Kubernetes **NetworkPolicy** provides segmentation.
...
**SUSE Virtualization:** **No equivalent** — there is no NSX-style product lineage; VM L2 uses VLAN-backed **Multus** networks and the **Canal** CNI handles pod traffic.
...
**SUSE Virtualization:** **No Geneve VM overlay** — the pod **Canal** CNI uses a VXLAN overlay for pod traffic, but VM L2 networks are **VLAN-backed via Multus**, not an overlay.
...
**SUSE Virtualization:** The closest analog is a **VM Network** (a VLAN-backed Multus `NetworkAttachmentDefinition`), an L2 domain — but VLAN-scoped, not a Geneve-backed overlay segment.
...
**SUSE Virtualization:** **No direct equivalent** — external routers/firewalls handle north-south routing; Harvester provides only an **L4 load balancer**, not tiered logical gateways.
...
**SUSE Virtualization:** Kubernetes **NetworkPolicy** provides east-west micro-segmentation, the closest analog to the distributed firewall (though scoped to pods/attached workloads).
...
**SUSE Virtualization:** **No direct equivalent** — there is no NSX Edge appliance; north-south services rely on external gateways/load balancers plus Harvester's built-in **L4 LB**.
...
**SUSE Virtualization:** **No equivalent** — Harvester has no NSX host switch; VM networking comes from **ClusterNetworks** and **Multus**, with **Canal** as the pod CNI.
...
**SUSE Virtualization:** **No direct equivalent** — a **ClusterNetwork** defines which nodes and uplinks a VM network spans, loosely paralleling a VLAN transport zone.
...
**SUSE Virtualization:** **No equivalent** — network config is managed declaratively through **Rancher/Harvester** Kubernetes CRDs, not a dedicated SDN manager plane.
...
**SUSE Virtualization:** Kubernetes **NetworkPolicy** with **label selectors** provides tag-driven, workload-following segmentation, though without NSX's full group/policy model.
...
**SUSE Virtualization:** Harvester roughly matches the **vDS-level** capability (VLAN VM networks via ClusterNetwork/Multus) and adds **NetworkPolicy**, but has **no overlay/distributed-routing/SDN** layer like NSX.
