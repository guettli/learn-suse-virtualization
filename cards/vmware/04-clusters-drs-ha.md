...
**SUSE Virtualization:** The **Harvester cluster** itself pools its nodes' CPU, memory, and storage via Kubernetes, enabling cluster-wide features like live migration and VM high availability.
...
**SUSE Virtualization:** The **Kubernetes scheduler** handles VM placement, but there is **no full DRS** — no continuous, threshold-driven load-balancing of already-running VMs.
...
**SUSE Virtualization:** No direct equivalent — the scheduler makes **initial placement** automatically, and there are no manual/partial/full tiers governing ongoing rebalancing.
...
**SUSE Virtualization:** No direct equivalent — with no DRS-style load balancer, there is no migration-threshold or aggressiveness slider.
...
**SUSE Virtualization:** The **Kubernetes scheduler** performs **initial placement**, but automatic ongoing load-balancing via live migration (DRS-style) is not provided.
...
**SUSE Virtualization:** A **namespace with a ResourceQuota** (surfaced as a Rancher **project**) partitions cluster capacity for a group of VMs or a team.
...
**SUSE Virtualization:** VMs use pod-style **requests and limits**, with the cluster **overcommit configuration** governing how far CPU/memory can be oversubscribed; there is no separate shares-weighting mechanism.
...
**SUSE Virtualization:** **VM high availability** — KubeVirt automatically **restarts VMs from a failed node onto healthy nodes** (requires replicated Longhorn storage) — is the equivalent HA restart.
...
**SUSE Virtualization:** No FDM election — node-failure detection and VM restarts are handled by standard **Kubernetes control-plane controllers** (node controller plus KubeVirt), backed by the **etcd** quorum.
...
**SUSE Virtualization:** No direct equivalent — no capacity is pre-reserved for failover; a VM simply **fails to schedule** if the surviving nodes lack the capacity to run it.
...
**SUSE Virtualization:** The analog is Kubernetes **node NotReady** detection; there is no configurable isolation response — VMs are rescheduled once the node is judged failed.
...
**SUSE Virtualization:** No datastore-heartbeat channel — node liveness is judged by **kubelet heartbeats to the API server**, with the **etcd** quorum guarding against split-brain.
...
**SUSE Virtualization:** No direct equivalent — there is no vendor-health-driven proactive evacuation; **node maintenance mode** is the manual way to drain a node before servicing it.
...
**SUSE Virtualization:** Live-migration CPU compatibility is handled by KubeVirt's **CPU model (e.g. `host-model`) and compatibility checks**, the closest analog to an EVC baseline.
...
**SUSE Virtualization:** No direct equivalent — Harvester has no automated power-management feature that consolidates VMs and powers idle nodes off.
...
**SUSE Virtualization:** KubeVirt exposes Kubernetes **node affinity** and **pod affinity/anti-affinity** rules to keep VMs together, apart, or pinned to specific nodes.
...
**SUSE Virtualization:** No direct equivalent — there is no forecast-driven predictive migration of VMs.
