...
**SUSE Virtualization:** Access is **Kubernetes RBAC** — a **Role/ClusterRole** (a bundle of API verb+resource rules) is tied to a subject by a **RoleBinding**, with **Rancher** roles/projects layered on once the cluster is imported.
...
**SUSE Virtualization:** The RBAC analog is **cluster-wide vs namespace-scoped** — a **ClusterRoleBinding** grants across the whole cluster, while a namespaced **RoleBinding** limits access to one namespace.
...
**SUSE Virtualization:** **Rancher authentication** is the broker once the cluster is imported, supporting **AD/LDAP/SAML/OIDC** providers; a standalone Harvester uses local authentication.
...
**SUSE Virtualization:** No direct toggle — the host is an **immutable SLE Micro appliance** managed through the cluster API, so there is no separate lockdown mode to enable/disable.
...
**SUSE Virtualization:** **Encrypted Longhorn volumes** (LUKS) provide at-rest encryption — you create an encrypted **StorageClass** referencing a secret, and VM disks on it are encrypted transparently.
...
**SUSE Virtualization:** No built-in Native-Key-Provider analog — encryption keys for Longhorn volumes live in **Kubernetes Secrets** (referenced by the encrypted StorageClass), which you manage/back up yourself rather than an integrated in-cluster KMS.
...
**SUSE Virtualization:** KubeVirt exposes a guest **vTPM** and **UEFI Secure Boot** (via OVMF/SMM firmware) so VMs can run BitLocker/Windows security features and verify their boot chain.
...
**SUSE Virtualization:** No embedded CA product — cluster TLS is handled by **cert-manager**, and you set the UI/API certificate through the Harvester **ssl-certificates** setting.
...
**SUSE Virtualization:** The host equivalent is **UEFI Secure Boot on the SLE Micro node**, verifying the bootloader/kernel of the immutable host OS itself (distinct from the KubeVirt guest vTPM/Secure Boot).
...
**SUSE Virtualization:** Guest VBS depends on the same building blocks — **KubeVirt vTPM, UEFI Secure Boot, and IOMMU** — exposed through KubeVirt VM features where the platform supports them.
...
**SUSE Virtualization:** The same principle applies via **Kubernetes RBAC** — give each integration a dedicated **ServiceAccount** bound to a narrowly scoped Role rather than cluster-admin, limiting blast radius and keeping actions auditable.
