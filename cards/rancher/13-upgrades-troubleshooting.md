...
A: Pull the management logs with `kubectl -n cattle-system logs -l app=rancher` (leader pod is most useful), plus **cattle-cluster-agent** logs in the affected downstream cluster. For deep diagnostics use the **`rancher/support-tools`** / support-bundle collector, and gather RKE2/K3s server logs (`journalctl -u rke2-server`) on the local cluster nodes.
